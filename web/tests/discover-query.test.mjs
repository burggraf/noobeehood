...
import { canonicalDiscoverPage, discoverUrl, readDiscoverParams } from '../src/lib/discover-query.js';
...
test('canonicalizes pages beyond available results, including empty directories', () => {
  assert.equal(canonicalDiscoverPage(4, 3), 3);
  assert.equal(canonicalDiscoverPage(4, 0), 1);
  assert.equal(canonicalDiscoverPage(1, 0), 1);
});
